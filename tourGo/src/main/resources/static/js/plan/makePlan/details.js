...
function createItem(result, parentEl) {
	...
	for (const item of result) {
		...
		<img src="${item.firstimage}" class="img-event" onerror="this.onerror=null; this.src='../../../../images/noimg.jpg';"></img>
		</div>
		`;
		html += `
								<div>${item.title}</div>
		...
		label.addEventListener("click", function() {
			...
			if (kakao && map) {
				...
				// 마커를 클릭했을 때 마커 위에 표시할 인포윈도우를 생성합니다
				var iwContent = `<div style="padding:5px;">${infoTitle}</div>
				<img class="imgview img-event" src="${infoImg}" onerror="this.onerror=null; this.src='../../../../images/noimg.jpg';"></img>`
								, // 인포윈도우에 표출될 내용으로 HTML 문자열이나 document element가 가능합니다
   				 iwRemoveable = true; // removeable 속성을 ture 로 설정하면 인포윈도우를 닫을 수 있는 x버튼이 표시됩니다
